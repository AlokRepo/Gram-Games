...
const FallingLeavesBackground: React.FC = () => {
  const [leafStyles, setLeafStyles] = useState<LeafStyleProps[]>([]);
  ...
  useEffect(() => {
    ...
    setLeafStyles(generateStyles());
  }, []); // Empty dependency array ensures this runs once on mount (client-side)

  // Don't render anything until styles are generated on the client
  if (leafStyles.length === 0) {
    return null;
  }
